 ...
 *                 If `new_size` is 0, the function behaves like `ft_free(ptr)`
 *                 and returns NULL.
 *
...
void *ft_realloc(void *ptr, size_t old_size, size_t new_size)
{
	...
	if (!ptr) {
		if (old_size != 0) {
			printf("WARNING: realloc called with ptr=NULL but old_size=%u (ignored)\n",
				(uint32_t)old_size);
		}
	...
	if (actual_size == 0) {
		printf("ERROR: realloc on invalid pointer 0x%x\n", (uint32_t)ptr);
		return NULL;
	}
	...
	if (!new_ptr) {
		printf("ERROR: realloc vmalloc(%u) failed\n", (uint32_t)new_size);
		return NULL;
	}
