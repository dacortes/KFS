...
/**
 * @brief Print the memory map provided by the bootloader.
 *
 * This function prints the memory map information provided by the bootloader
...
void pmm_print_memory_map(multiboot_info_t *info) {
	...
	for (; (uint32_t)entry < info->mmap_addr + info->mmap_length;
		...
		printf("%5s | 0x%08x | 0x%08x | %d MB\n", 
			type_str, (uint32_t)base, (uint32_t)end, (uint32_t)(length / (1024 * 1024)));
	}
